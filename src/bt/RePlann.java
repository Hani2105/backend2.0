...
//ha idáig eljutunk ,létrehozunk egy új po-t és kitöltjük a megfelelő adatokkal,és a megmaradt mennyiséggel
                    PlannObject po = new PlannObject(p.getbackendSheet(), p.getWidth(), p.getHeight(), p.getPn(), p.getJob(), vt.getVtstartdate(), p.getTerv() - p.getTeny(), 0, p.getPlannerkomment(), p.getKomment(), p.isEngineer(), 0, p.getWorkStation(), p.getCiklusido(), p.getMainWindow());
