...
//kell egy query a 
        ...
        String query = "update tc_terv set active = CASE when tc_terv.active = 2 then 1 when tc_terv.active = 1 then 0 end where tc_terv.active in (2,1) and tc_terv.date between '" + dateFormat.format(b.getM().cp.jDateChooser1.getDate()) + "' and '" + dateFormat.format(b.getM().cp.jDateChooser2.getDate()) + "' and tc_terv.idtc_becells = (select tc_becells.idtc_cells from tc_becells where tc_becells.cellname ='" + b.getName() + "')";
        PlanConnect pc = null;
        try {
            pc = new PlanConnect();
            pc.feltolt(query);
